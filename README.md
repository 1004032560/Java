...
3、`TCP`（传输控制协议）与`UDP`（用户数据报协议）的区别
...
5、`IP`地址：是`IP`协议提供的一种统一的地址格式，它为互联网上的每一个网络和每一台主机分配一个逻辑地址

6、端口号：是用来区分各种服务。一台计算机上有65536个端口号，已经被操作系统占用的0-1024；例如：`http:80`，使用前需要获得管理员权限，尽量不用

7、网络号的标识就是子网

8、`Java`语言对`TCP`协议网络通信使用`java.net`包中的`Socket`和`ServerSocket`进行支持；`Socket`被称为“套接字”，用来描述`IP`地址和端口
